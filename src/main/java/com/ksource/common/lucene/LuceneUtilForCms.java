...
/**
 * 此类为 lucene 索引工具类
 * 此类只是对索引进行增，删，改，查操作，不包括Lucene架构的配置及索引文件配置。有关配置的信息在LuceneFactoryForCms类。
 ...
 */
@Service
public class LuceneUtilForCms {
	private static JsonMapper binder = JsonMapper.buildNonNullMapper();
    //日志
    private static final Logger log = LogManager.getLogger(LuceneUtilForCms.class);
    ...
    public static <T> ServiceResponse createIndex(List<T> queryLayInfos,
        ...
        //创建之前先删除索引
        LuceneFactoryForCms.deleteIndexDir();
        if (log.isDebugEnabled()) {
            log.debug("删除旧索引成功...");
        }
        IndexWriter writer = LuceneFactoryForCms.getIndexWriter();
        try {
            int count = 0;
     ...
     * {@link LuceneUtilForCms#queryIndex(Object, String, String[], String, String, HttpSession)}
     * @param <T> 
                ...
                if ((obj != null) && !"null".equals(doc.get(fieldName))) {
                    ...
                    if ((obj1 != null) && (list != null) &&
                        ...
                        obj1 = addHighligh(fieldName, obj1.toString(),
                                LuceneFactoryForCms.getSessionQuery(session, queryType));
                    }
    ...
    public static<T> void updateIndex(T layInfo, String flag,
        ...
        IndexWriter writer = LuceneFactoryForCms.getIndexWriter();
        try {
            writer.updateDocument(new Term(flag,String.valueOf(binder.beanToMap(layInfo).get(flag))),
	...
	public static<T> PaginationHelper<T> queryIndex(T domain,
        ...
        try {
            ...
            searcher = LuceneFactoryForCms.getIndexSearcher();
            Integer start = (Integer) map.get("start");
            Integer end = (Integer) map.get("end");
            map.remove("start");
            map.remove("end");

            Query query = LuceneFactoryForCms.createQuery(map, session,
    ...
    public static <T> T detail(String infoId, String flag, Class<T> cla,
        ...
        IndexSearcher searcher = LuceneFactoryForCms.getIndexSearcher();
   ...
   private static void closeWriter(IndexWriter writer){
    ...
    private static String addHighligh(String fieldName, String text, Query query) {
        ...
        if ((query != null) && query.toString().contains(fieldName)) {
            SimpleHTMLFormatter simpleHTMLFormatter = new SimpleHTMLFormatter(LuceneFactoryForCms.HIGHLIGH_FLAG_START,
                    LuceneFactoryForCms.HIGHLIGH_FLAG_END);
            Highlighter highlighter = new Highlighter(simpleHTMLFormatter,
                    new QueryScorer(query));
            if(fieldName.equals("content")){
            	highlighter.setTextFragmenter(new SimpleFragmenter(text.length()));    
            }
            TokenStream tokenStream = LuceneFactoryForCms.getAnalyzerInstance()
    ...
    public static void removeSessionQuery(HttpSession session, String queryType) {
        LuceneFactoryForCms.removeSessionQuery(session, queryType);
    }
}
