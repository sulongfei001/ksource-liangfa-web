...
public class LuceneFactory {
    ...
    public static final String INDEX_FILE_PATH="resources/index/lay";
    ...
    String[] QUERY_TITLE = new String[]{"title","typeId"};
    String[] QUERY_CONTENT = new String[]{"content","typeId"};
    String[] QUERY_ALL = new String[]{"title","content","typeId"};
    ...
    private static Query createParseQuery(Entry<String, Object> entry){
         try {
			return IKQueryParser.parseMultiField(new String[]{"title","content","typeId"},entry.getValue().toString());
		} catch (IOException e) {
			e.printStackTrace();
		}
		return null;
	...
	public static IndexSearcher getIndexSearcher(String string) {
		
		IndexSearcher searcher=null;
		try{
		Directory dir = getLuceneDirectory(string);
	      searcher = new IndexSearcher(dir);
	      searcher.setSimilarity(new IKSimilarity());
		}catch (IOException e) {
		      System.out.println(" caught a " + e.getClass() +
	...
	private static Directory getLuceneDirectory(String string) {
	...
	private static String getRealPath(String searchChick) {
		String separator = "/";
    	String separatorTwo ="\\\\";
    	String savePath = INDEX_FILE_PATH;
    	String realpath = SystemContext.getRealPath();
    	...
    	realpath += path;
    	if(StringUtils.isBlank(searchChick)){
    		realpath += searchChick;
    	}
		return realpath;
	}
