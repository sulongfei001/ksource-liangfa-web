	...
	<script type="text/javascript" src="${path }/resources/jquery/zTree/v35/jquery.ztree.core-3.5.min.js"></script>
 	<script type="text/javascript">
		var setting = {
			...
			async: {
				enable: true,
				url: "${path}/system/district/loadDistrictTreeForInstruction?searchRank=${searchRank}",
				autoParam: ["id"],
			},
			callback: {
		...
		$(function(){
			...
			districtTree=$.fn.zTree.init($("#SEARCH_BY_DISTRICT"),setting);
 		    handleSelects();
 		    $("#orgListFrame").attr("src","${path}/system/org/findByDistrictCode?isSingle=${isSingle }&searchRank=${searchRank}");
		});
		...
		function districtTreeOnClick(event, treeId, treeNode){
			if (treeNode) {
			var url="${path}/system/org/findByDistrictCode?districtCode="+treeNode.id+"&isSingle="+isSingle+"&flag=0&searchRank=${searchRank}";
		...
		function selectOrg(){
			...
			if (orgIds.length == 0) {
				$.ligerDialog.warn("您没有选择任何接收单位，请重新选择 ！");
				return;
			} else {
