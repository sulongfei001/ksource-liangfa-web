 	...
 	<script type="text/javascript">
		...
		function load_district_tree(){
			districtTree=$('#SEARCH_BY_DISTRICT').zTree({
				...
				async: true,
				asyncUrl:"${path}/system/districtExternal/loadDistrictTree?searchRank=${searchRank}",
				asyncParam: ["id"],
				callback: {
		...
		function districtTreeOnClick(event, treeId, treeNode){
			if (treeNode) {
			var url="${path}/system/organiseExternal/findCommunionByDistrictCode?&districtCode="+treeNode.id+"&isSingle="+isSingle+"&flag=0"+"&searchRank=${searchRank}";
...
<div class="panel">
	<div id="defLayout">
		...
		<div position="center" title="组织机构">
			 <iframe id="orgListFrame" name="orgListFrame" height="90%" width="100%" frameborder="0"src="${path}/system/organiseExternal/findCommunionByDistrictCode?isSingle=${isSingle }&searchRank=${searchRank}" ></iframe> 
		</div>
