...
$(function(){
	...
	function search(){
		 ...
		 $('#findForm')[0].action="${path}/breport/caseFilterStatsByYearCity?indexList="+index;
		 $('#findForm').submit();                
	}
	$(function(){
		hiddenDIV();
...
<legend class="legend">线索筛查案件统计汇总(按年度)</legend>
	...
	<table align="center" style="width: 100%;">
		<tr>
			<td align="right">
				...
				<report:html name="report1" reportFileName="business_caseFilterStatsByYearCity.raq"
					funcBarLocation=""
					needPageMark="yes"
...
<div id="indexDIV" style="display:none; position:absolute; height:210px;width:23%; background-color:white;border:1px solid #ABC1D1;overflow-y:auto;overflow-x:auto;">
	...
	<div align="left">
		<input type="hidden" name="index" value="${indexList}"></input>
		<input type="checkbox" name="indexList" value ="M" <c:if test="${fn:contains(indexList,'M')}">checked="checked"</c:if>/><span class="indexSpan">行政处罚案件</span> <br/>
		<input type="checkbox" name="indexList" value ="N" <c:if test="${fn:contains(indexList,'N')}">checked="checked"</c:if>/><span class="indexSpan">疑似犯罪案件</span> <br/>
		<input type="checkbox" name="indexList" value ="A" <c:if test="${fn:contains(indexList,'A')}">checked="checked"</c:if>/><span class="indexSpan">行政处罚2次以上</span> <br/>
		<input type="checkbox" name="indexList" value ="B" <c:if test="${fn:contains(indexList,'B')}">checked="checked"</c:if>/><span  class="indexSpan">涉案金额达到刑事追诉标准80%以上</span> <br/>
		<input type="checkbox" name="indexList" value ="C" <c:if test="${fn:contains(indexList,'C')}">checked="checked"</c:if>/><span  class="indexSpan">有过鉴定</span> <br/>
		<input type="checkbox" name="indexList" value ="E" <c:if test="${fn:contains(indexList,'E')}">checked="checked"</c:if>/><span  class="indexSpan">经过负责人集体讨论</span> <br/>
		<input type="checkbox" name="indexList" value ="F" <c:if test="${fn:contains(indexList,'F')}">checked="checked"</c:if>/><span  class="indexSpan">情节严重</span> <br/>
		<input type="checkbox" name="indexList" value ="D" <c:if test="${fn:contains(indexList,'D')}">checked="checked"</c:if>/><span  class="indexSpan">处以行政处罚规定下限金额以下罚款</span> <br/>
	</div> 
</div>
</div>
</body>
