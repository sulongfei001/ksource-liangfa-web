...
var zTreeSetting = {
	...
	//在显示行政区划信息之前先查询该行政区划是否存在，如果不存在刷新节点
	 ...
	 $.getJSON('<c:url value="/system/district/checkId/"/>'+id,function(data){
				 if(data.result==true){
				     window.frames['taskAssignFrame'].location.href='<c:url value="/taskAssignSetting?division=taskAssign&districtCode="/>'+id;
				 }else{
					 var dialog = $.ligerDialog.waitting(data.msg);
					 setTimeout(function(){
						 dialog.close();
					 },2000);
...
<body>
	<div class="panelC">
		<div id="layout">
			<div title="任务办理设置" position="left">
				<ul id="treeC" class="ztree" style="overflow:auto;"></ul>
			</div>
			<div position="center">
				<iframe id="taskAssignFrame" name="taskAssignFrame" class="ui-layout-center" width="99%" height="99%" src="" frameborder="0"></iframe>
			</div>
		</div>
	</div>
</body>
