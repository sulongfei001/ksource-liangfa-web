		...
		<table class="blues"  style="margin: 10px;width:96%; float: left;">
			...
			<tr><td class="tabRight">移送起诉罪名：</td><td colspan="5" style="text-align: left;">
				<c:forEach items="${xianyiren.accuseInfoList }" var="accuseInfo" varStatus="accuseInfo_state">
					<a id="${stepId }_accuseInfo_${xianyiren_state.index}_${accuseInfo_state.index }" title="${fn:escapeXml(accuseInfo.law)}" class="accuseInfo">${accuseInfo.name }(${accuseInfo.clause })</a>&nbsp;&nbsp;<c:if test="${!accuseInfo_state.last }"><br/></c:if>
				</c:forEach>
			</td></tr>
		</table>
		</c:forEach>
