...
<h3>>>要求公安说明不立案理由</h3>
		<table class="blues" style="width: 98%;margin-left: 10px;margin-top: 0px;">
			...
			<tr>
				...
				<td width="80%" style="text-align: left;" colspan="3">
					<fmt:formatDate value="${require.requireTime}" pattern="yyyy-MM-dd"/>
			</td>
			</tr>
			<tr>
				<td width="20%"  class="tabRight" >要求说明的问题：</td>
				<td width="80%" style="text-align: left;" colspan="3">
					${require.requireQuestion}
				</td>
			</tr>
			<tr>
				<c:set var="attachFileKey" value="f${require.attachFile}"></c:set>
						...
						<c:if test="${attaMap[attachFileKey].canOnlineRead}">
				            <c:choose>
				                ...
				                <c:when test="${!empty attaMap[attachFileKey].swfPath}">
				                    <a style=" margin-left: 4px;"
				                       href="javascript:void(0);"
				                       onclick="docPreview('${require.attachFile}','${attaMap[attachFileKey].attachmentName}');"
				                       title="在线查看"> 预览
				                    </a>
				                </c:when>
				            </c:choose>
