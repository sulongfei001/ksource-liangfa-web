...
<h3>>>行政不予立案</h3>
		...
		<tr>
			<td width="20%"  class="tabRight" >不立案理由：</td>
			<td width="80%" style="text-align: left;" colspan="3">
		...
		<c:set var="notLiAnAttachId" value="f${xingzhengNotLiAnForm.attachId}"></c:set>
			...
			<tr>
				...
				<td width="80%" style="text-align: left;"  colspan="3">
			        ...
			        <span>
			        <c:if test="${!empty attaMap[notLiAnAttachId]}">
			        <a style=" height: 32px; padding-top: 10px;" href="${path}/download/caseAtta/${xingzhengNotLiAnForm.attachId}"
			           title="下载">
			            下载
			        </a>
			        </c:if>
					<c:if test="${attaMap[notLiAnAttachId].canOnlineRead}">
			            <c:choose>
			                ...
			                <c:when test="${!empty attaMap[notLiAnAttachId].swfPath}">
			                    <a style=" margin-left: 4px;"
			                       href="javascript:void(0);"
			                       onclick="docPreview('${xingzhengNotLiAnForm.attachId}','${attaMap[notLiAnAttachId].attachmentName }');"
			                       title="在线查看"> 预览
			                    </a>
			                </c:when>
			            </c:choose>
