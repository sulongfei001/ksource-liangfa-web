		...
		<table class="blues" style="width: 98%;margin-left: 10px;margin-top: 0px;">
			<tr>
				...
				<td width="80%" style="text-align: left;" colspan="3">
					${requireLian.undertaker}
				</td>
			</tr>
			<tr>
				<td width="20%"  class="tabRight">承办时间：</td>
				<td width="80%" style="text-align: left;" colspan="3">
					<fmt:formatDate value="${requireLian.requireTime}" pattern="yyyy-MM-dd"/>
			</td>
			</tr>
			<tr>
				<c:set var="attachFileKey" value="f${requireLian.attachFile}"></c:set>
						...
						<c:if test="${attaMap[attachFileKey].canOnlineRead}">
				            <c:choose>
				                ...
				                <c:when test="${!empty attaMap[attachFileKey].swfPath}">
				                    <a style=" margin-left: 4px;"
				                       href="javascript:void(0);"
				                       onclick="docPreview('${requireLian.attachFile}','${attaMap[attachFileKey].attachmentName}');"
				                       title="在线查看"> 预览
				                    </a>
				                </c:when>
				            </c:choose>
			...
			<tr>
				<td width="20%"  class="tabRight" >附件说明：</td>
				<td width="80%" style="text-align: left;" colspan="3">
					${requireLian.fileExplain}
				</td>
			</tr>
			<tr>
