...
<h3>>>行政拘留，已结案</h3>
		...
		<tr>
			...
			<td width="80%" style="text-align: left;" colspan="3">
				${detentionInfo.undertaker }
			</td>
		</tr>
		<tr>
			<td width="20%"  class="tabRight" >承办时间：</td>
			<td width="80%" style="text-align: left;" colspan="3">
				<fmt:formatDate value="${detentionInfo.undertakeTime }" pattern="yyyy-MM-dd"/>
			</td>
		</tr>
		<tr>
			<td width="20%"  class="tabRight" >拘留天数：</td>
			<td width="80%" style="text-align: left;" colspan="3">
				${detentionInfo.detentionDays}天
			</td>
		</tr>
		<c:set var="detentionFileKey" value="f${detentionInfo.detentionFile}"></c:set>
			<tr>
				<td width="20%"  class="tabRight" >拘留决定书：</td>
				<td width="80%" style="text-align: left;"  colspan="3">
					${attaMap[detentionFileKey].attachmentName }
			        <span>
			        <c:if test="${!empty attaMap[detentionFileKey]}">
			        <a style=" height: 32px; padding-top: 10px;" href="${path}/download/caseAtta/${detentionInfo.detentionFile}"
			           title="下载">
			            下载
			        </a>
			        </c:if>
					<c:if test="${attaMap[detentionFileKey].canOnlineRead}">
			            <c:choose>
			                <c:when test="${empty attaMap[detentionFileKey].swfPath}">
			                ...
			                <c:when test="${!empty attaMap[detentionFileKey].swfPath}">
			                    <a style=" margin-left: 4px;"
			                       href="javascript:void(0);"
			                       onclick="docPreview('${detentionInfo.detentionFile}','${attaMap[detentionFileKey].attachmentName }');"
			                       title="在线查看"> 预览
			                    </a>
			                </c:when>
			            </c:choose>
			...
			<tr>
			<td width="20%"  class="tabRight" >需要说明的问题：</td>
			<td width="80%" style="text-align: left;" colspan="3">
				${detentionInfo.memo }
			</td>
		</tr>
