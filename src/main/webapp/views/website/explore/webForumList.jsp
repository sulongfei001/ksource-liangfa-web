...
<div>
<p class="address">你所在的位置：<a class="font_12_0075b8" href="javascript:home(1);">首页</a> >> ${forumCommunity.name }</p>
<div class="sjz_w739 fl">
<div class="lfltmenunew2"><h3 class="fl">${forumCommunity.name }</h3></div>
<div class="sjz_listcont1">
<div id="webForum">
	<display:table name="forumThemes" uid="forumTheme" requestURI="${path }/website/explore/selectWebForumMain">
		<display:column class="w620">
			<a href="javascript:;" onclick="selectWebForumDetail('${forumTheme.id }',${index},${searchId})" title="${forumTheme.name}">${fn:substring(forumTheme.name,0,30)}${fn:length(forumTheme.name)>30?'...':''}</a>
		</display:column>
		<display:column class="w80">
			[<fmt:formatDate value="${forumTheme.createTime }" pattern="yyyy-MM-dd" />]
		</display:column>
	</display:table>
</div>
</div>
