...
<div class="lfltmenunew2"><h3>${webPrograma.programaName }</h3></div>
<div class="sjz_listcont1 detailcont">
<h3>${webArticle.articleTitle }</h3>
<div class="time"><span class="fl textright">发表日期：<fmt:formatDate value="${webArticle.articleTime }" pattern="yyyy-MM-dd"/></span>    <span class="fr textleft">撰文：${user.userName }</span> <div class="clear"></div></div>
<c:if test="${not empty webArticle.imagePath }">
<center>
<img id="image" style="display: none;" src="${webArticle.imagePath }"/>
</center>
</c:if>
