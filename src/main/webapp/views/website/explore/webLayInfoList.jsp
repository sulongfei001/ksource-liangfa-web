...
<div>
<p class="address">你所在的位置：<a class="font_12_0075b8" href="javascript:home(1);">首页</a> >> 法律法规</p>
<div class="sjz_w739 fl">
<div class="lfltmenunew2"><h3 class="fl">法律法规</h3></div>
<div class="sjz_listcont1">
<div id="layInfo">
	<display:table name="helpers" uid="helper" requestURI="${path }/website/explore/selectLayInfoMain">
		<display:column class="w620">
			<a href="javascript:;" onclick="webLayInfoDetail(${helper.infoId },9,${searchId})" title="${helper.title}">${fn:substring(helper.title,0,30)}${fn:length(helper.title)>30?'...':''}</a>
		</display:column>
		<display:column class="w80">
			[<fmt:formatDate value="${helper.createTime }" pattern="yyyy-MM-dd" />]
		</display:column>
	</display:table>
</div>
</div>
