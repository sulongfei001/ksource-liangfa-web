...
NTKO.addSignFromURL = function (key)
{
    ...
    {
        $.ligerDialog.warn("请从文件柜或网络硬盘选择印章!");
        return;
    }
...
NTKO.addDocHeader = function (url,docType,ext)
{
    ...
    {
        $.ligerDialog.warn(err.number + ":" + err.description);
    }
}
...
NTKO.setReadOnly = function (boolvalue)
{
    ...
    try
    {
        ...
        $.ligerDialog.warn("错误：" + err.number + ":" + err.description);
    }
    finally{
    }
}
...
NTKO.saveDoc = function (op_flag)
{
    ...
    try
    {
        ...
        if(!paraObj.FFN)
        {
            $.ligerDialog.warn("参数错误：控件的第二个参数没有指定。");
            return;
        }
        if(!ntkoObj.docOpen)
        {
            $.ligerDialog.warn("没有打开的文档。");
            return;
        }
        switch(ntkoObj.op)
        {
            case "1":
                ...
                if(retStr == 'OK') {
                    window.$.ligerDialog.warn('文档 '+ "'" + ntkoObj.fileName+ "'" + ' 保存成功！');
                //ntkoObj.docOpen = false;
                }
                break;
            case "2":
            ...
            case "6":
            case "7":
                $.ligerDialog.warn("文档处于阅读状态，您不能保存到服务器。");
            default:
                break;
        }
    }
    catch(err){
        $.ligerDialog.warn( "不能保存到URL：" + err.description);//"不能保存到URL："
    }
    finally{
    }
}
...
NTKO.savePDFToServer = function ()
{
    ...
    try
    {
        ...
        $.ligerDialog.warn( "不能保存PDF到URL："+ err.number + ":" + err.description);//"不能保存PDF到URL："
    }
    finally{
    }
}
...
NTKO.convertToAip = function() {
    ...
    if (_originalConfirm('确认要将正文转为版式文件？')) {
        if ($(window.parent.document).find('#tab_aip_frame').length > 0) {
            ...
            $.ligerDialog.warn("无版式文件显示权限，请联系管理员！");
        }
    }
}
