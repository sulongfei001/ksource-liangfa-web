...
OfficePlugin={
		...
		init:function(){
			...
			var fileId=this.fileObjs.get(0).getAttribute("value");
			doctype = 'doc';
			if ($.browser.msie) {
				//有读和写的权限，加载控件。
				//$.ligerDialog.waitting('正在加载OFFICE控件,请稍候...');
				//加载控件。
				var officeObj= new OfficeControl();
				this.officeObjs.push(officeObj);
				//加载office控件。
				officeObj.renderTo("office-div",{fileId:fileId,doctype:doctype,myNum:myNum});
				//是否有office控件。
				...
				//$.ligerDialog.warn("未装载文档控件，请设置好IE安全级别为中或中低，不支持非IE内核的浏览器。");
			}
			//this.initAIP();
		},
		initAIP:function(){
			//加载AIP组件
			this.aipObjs=$("input[controltype='aip']");
			//for ( var i = 0; i < aipObjs.length; i++) {
				var ao = this.aipObjs.get(0);
				var name=ao.getAttribute("name");
				var fileId=ao.getAttribute("value");
				
				var width=$(ao).css('width');
				var height=$(ao).css('height');
				...
				//字段权限
				var right="w";//fileObj.getAttribute("right");
				//默认菜单权限
				...
				//字段对应的控件的菜单权限
				var menuRightStr = ao.getAttribute("menuRight");
				...
				if(right=="no"){
					...
					if ($.browser.msie) {
					//有读和写的权限，加载控件。
					//$.ligerDialog.waitting('正在加载AIP,请稍候...');
					//加载控件。
					this.aipObj = new AIP();
					var officeObj= this.aipObj;
					//this.officeObjs.push(officeObj);
					//加载office控件。
					officeObj.renderTo(divId,{fileId:fileId,doctype:doctype,right:right,user:this.user,menuRight:menuRight});
					//是否有office控件。
					//this.hasOfficeFields.push(true);
					//office控件文档标志
					//this.hasSubmitOffices.push(false);
...
/*			var aipObj = this.aipObj;
			var right="w";//this.fileObjs.get(cn).getAttribute("right");
			//可写，保存office内容并上传。
			if(right=="w"||right=="b"){
				//保存到服务器。
				var result=aipObj.saveRemote();
				if(result==-11){
				}else if(result==-13){
					$.ligerDialog.warn("提交失败,AIP控件没能正常使用，请重新安装 ！！！","提示");
				}else if(result == 0){	
				}else{
					if(result>0){
						this.aipObjs.get(0).setAttribute("value",result);
					}else{
					}
				}
			}*/
...
function sysFileType(fileId){
	var doctype = "doc";
	var path = top.APP_PATH +'/platform/system/sysFile/getFileType.ht';
...
function getDocUserData(){
	...
	var path = top.APP_PATH +'/platform/system/sysFile/getUserData.ht';
    $.ajaxSetup({async:false});  //同步
