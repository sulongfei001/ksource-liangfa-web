    ...
    if(ServletFileUpload.isMultipartContent(request)){
    	    ...
    	    while(fii.hasNext()){
    	        FileItemStream fis = fii.next();
    	            ...
    	            if(!fis.isFormField() && fis.getName().length()>0){
    					...
    					ftype = matcher.group();
    					newFileName = new Date().getTime()+ftype;
        ...
        response.getWriter().print("{'original':'"+originalName+"','url':'/"+filePath.substring(filePath.lastIndexOf("/")+1,filePath.length())+"/"+newFileName+"','title':'"+fileName+"','state':'"+state+"'}");
