    ...
    $.ligerui.controls.Accordion.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            $("> div", g.accordion).each(function (i, box)
            {
                ...
                if (i == selectedIndex){
                	//title 增加选择样式
                	header.addClass("l-accordion-header-selected");
                ...
                {
                	var imgsrc = $(box).attr("data-icon");
                    //默认第一次选中状态图标
                    if (i == selectedIndex){
                    	imgsrc = imgsrc.substring(0,imgsrc.lastIndexOf("."))+"__hover"+imgsrc.substring(imgsrc.lastIndexOf("."));
                    }
                    header.addClass("l-accordion-header-hasicon");
                    header.append('<i><img src="' + imgsrc + '" /></i>');
                }
            });
            ...
            $(">.l-accordion-header", g.accordion).click(function ()
            {
                ...
                if (togglebtn.hasClass("l-accordion-toggle-close"))
                {
                    ...
                    //title增加选中样式
                    $(this).addClass("l-accordion-header-selected");
                    $(this).siblings(".l-accordion-header").removeClass("l-accordion-header-selected");
                    //菜单图标
                    var $img = $(this).find("i > img");
                    var imgsrc =  $(this).next(".l-accordion-content").attr("data-icon");
                	imgsrc = imgsrc.substring(0,imgsrc.lastIndexOf("."))+"__hover"+imgsrc.substring(imgsrc.lastIndexOf("."));
                	$img.attr("src",imgsrc);
                	$(this).siblings(".l-accordion-header").each(function(){
                		var imgsrc =  $(this).next(".l-accordion-content").attr("data-icon");
                		$(this).find("i > img").attr("src",imgsrc);
                	});
                }
                else
                {
                    ...
                    //title取消选中样式
                    $(this).removeClass("l-accordion-header-selected");
                    //改变菜单图标
                    var $img = $(this).find("i > img");
                    var imgsrc =  $(this).next(".l-accordion-content").attr("data-icon");
                	$img.attr("src",imgsrc);
