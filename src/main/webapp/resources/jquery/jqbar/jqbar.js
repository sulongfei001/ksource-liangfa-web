        ...
        jqbar: function (options) {
            ...
            return this.each(function () {
                ...
                if (settings.orientation == 'h') {
                    ...
                    progressContainer.addClass('jqbar horizontal')
                    .append('<span class="bar-label"></span><span class="bar-level-wrapper"><span class="bar-level"></span></span><span class="bar-percent"></span>');
                    
                    var progressNo = progressContainer.find('.bar-no').html();
                    var progressLabel = progressContainer.find('.bar-label').html(settings.label);
                    var progressBar = progressContainer.find('.bar-level').attr('percent-value', settings.percent).attr('data-value', settings.value);;
                ...
                function animateProgressBar(progressBar) {
                    ...
                    var level = parseInt(progressBar.attr('percent-value'));
                    var dataValue = progressBar.attr('data-value');
                    if (level > 100) {
                        level = 100;
                    }
                    ...
                    if (settings.orientation == 'h') {
                        progressBar.animate({ width: w }, {
                            ...
                            step: function (currentWidth) {
                                ...
                                if (isNaN(percent))
                                    percent = 0;
                                progressContainer.find('.bar-percent').text(dataValue);
                            }
                        });
                    }
                    else {
